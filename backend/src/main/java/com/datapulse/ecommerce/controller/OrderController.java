...
public class OrderController {
    ...
    @GetMapping @Operation(summary="List orders") public ResponseEntity<ApiResponse<Page<OrderResponse>>> getOrders(@AuthenticationPrincipal UserPrincipal principal, @RequestParam(defaultValue="0") int page, @RequestParam(defaultValue="20") int size) {
        Pageable pageable = PageRequest.of(page,size,Sort.by("orderDate").descending());
        Page<OrderResponse> orders = principal.getRole().equals("ADMIN") ? orderService.getAllOrders(pageable) : orderService.getOrdersByUser(principal.getId(), pageable);
